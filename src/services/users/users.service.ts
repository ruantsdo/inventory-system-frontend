...
export async function getMyPermissions(): Promise<PermissionOutput[]> {
  const { data } = await apiClient.get<PermissionOutput[]>("/api/management/permissions");
  return data;
}

export async function getRoles(): Promise<RoleWithPermissionsOutput[]> {
  const { data } = await apiClient.get<RoleWithPermissionsOutput[]>("/api/management/roles");
  return data;
}
...
export async function createUser(
  ...
  const { data } = await apiClient.post<{ id: string; message: string }>("/api/users/", payload);
  return data;
}
